...
for epoch in range(epochs):   
    ...
    #用decoder生成新数据
    gen = model.decoder(sample)[0].view(img_size,img_size)

    plt.close("all")  # 关闭之前的图像

    #将测试步骤中的真实数据、重构数据和上述生成的新数据绘图
    concat = torch.cat((imgs[0].view(img_size, img_size),
            re_imgs[0].view(img_size, img_size), gen), 1)
...
sample = torch.randn(1,latent_size).to(device)
#用decoder生成新数据
generate = model.decoder(sample)[0].view(img_size,img_size)
#展示生成数据
